...
! function() {
  function i() {
    a = window.innerWidth, b = window.innerHeight, g = {
      x: 30 * a,
      y: 30 * b
    }, c = document.getElementById("canvas-wrapper"), d = document.getElementById("demo-canvas"), d.width = '1170', d.height = '500', e = d.getContext("2d"), f = [];
    ...
    for (var m = 0; m < f.length; m++) {
      for (var n = [], o = f[m], r = 0; r < f.length; r++) {
        var s = f[r];
        if (o != s) {
          for (var t = !1, u = 0; u < 5; u++) t || void 0 == n[u] && (n[u] = s, t = !0);
          for (var u = 0; u < 5; u++) t || q(o, s) < q(o, n[u]) && (n[u] = s, t = !0)
        }
      }
      o.closest = n
    }
    for (var m in f) {
      var v = new p(f[m], 2 + 2 * Math.random(), "rgba(255,255,255,0.3)");
      f[m].circle = v
    }
  }

  function j() {
    "ontouchstart" in window || window.addEventListener("mousemove", k)
  }

  function k(a) {
    var b = posy = 0;
    (a.pageX || a.pageY) && (b = a.pageX - 0, posy = a.pageY - 0), g.x = b, g.y = posy
  }

  function l() {
    m();
    for (var a in f) n(f[a])
  }

  function m() {
    if (h) {
      e.clearRect(0, 0, a, b);
      for (var c in f) Math.abs(q(g, f[c])) < 4e3 ? (f[c].active = .3, f[c].circle.active = .6) : Math.abs(q(g, f[c])) < 2e4 ? (f[c].active = .1, f[c].circle.active = .3) : Math.abs(q(g, f[c])) < 4e4 ? (f[c].active = .02, f[c].circle.active = .1) : (f[c].active = 0, f[c].circle.active = 0), o(f[c]), f[c].circle.draw()
    }
  ...
  function o(a) {
    if (a.active)
      for (var b in a.closest) e.beginPath(), e.moveTo(a.x, a.y), e.lineTo(a.closest[b].x, a.closest[b].y), e.strokeStyle = "rgba(156,217,249," + a.active + ")", e.stroke()
  }
  ...
  function p(a, b, c) {
    ...
    ! function() {
      ...
      d.active && (e.beginPath(), e.arc(d.pos.x, d.pos.y, d.radius, 0, 2 * Math.PI, !1), e.fillStyle = "rgba(156,217,249," + d.active + ")", e.fill())
    }
  }
